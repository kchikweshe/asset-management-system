...
class OAuthTestsMixin(object):
    ...
    def test_login(self):
        ...
        if resp_mocks is None:
            warnings.warn("Cannot tests provider %s, no oauth mock" % self.provider.id)
            return
        ...
        # The following lines don't actually tests that much, but at least
...
class OAuth2TestsMixin(object):
    ...
    def test_account_tokens(self, multiple_login=False):
        ...
        self.client.login(username=user.username, password="tests")
    ...
    def get_complete_parameters(self, q):
        return {"code": "tests", "state": q["state"][0]}
...
class SocialAccountTests(TestCase):
    ...
    def test_email_address_created(self):
        ...
        User = get_user_model()
        user = User()
        setattr(user, account_settings.USER_MODEL_USERNAME_FIELD, "tests")
        setattr(user, account_settings.USER_MODEL_EMAIL_FIELD, "tests@example.com")

        account = SocialAccount(provider="openid", uid="123")
        sociallogin = SocialLogin(user=user, account=account)
        complete_social_login(request, sociallogin)

        user = User.objects.get(**{account_settings.USER_MODEL_USERNAME_FIELD: "tests"})
        self.assertTrue(
    ...
    def test_email_address_clash_username_required(self):
        ...
        request, resp = self._email_address_clash("tests", "tests@example.com")
    ...
    def test_email_address_clash_username_not_required(self):
        ...
        self.assertNotEqual(user_username(user), "tests")
    ...
    def test_email_address_clash_username_auto_signup(self):
        ...
        request, resp = self._email_address_clash("tests", "other@example.com")
        self.assertEqual(resp["location"], "/accounts/profile/")
        user = get_user_model().objects.get(
            **{account_settings.USER_MODEL_EMAIL_FIELD: "other@example.com"}
        )
    ...
    def _email_address_clash(self, username, email):
        ...
        # Some existig user
        exi_user = User()
        user_username(exi_user, "tests")
        user_email(exi_user, "tests@example.com")
        exi_user.save()
    ...
    def test_disconnect(self):
        ...
        # Some existig user
        user = User()
        user_username(user, "tests")
        user_email(user, "tests@example.com")
        user.set_password("tests")
        user.save()
